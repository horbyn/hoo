...
set_cursor(int row, int col) {
    if (row >= CGA_HIGH || col >= CGA_WIDTH) {
        // cannot trace stack frame because it would make use of the cga
        hlt();
    }
    // 0x3b0-0x3bf MDA
    // 0x3c0-0x3cf CGA
...
scroll_back() {
    ...
    for (uint32_t i = 1; i <= CGA_HIGH - 1; ++i) {
        ...
        if (i == CGA_HIGH - 1) {
            ...
            for (uint32_t j = 0; j < CGA_WIDTH; ++j, ++pv)
                *pv = 0xf20;
        }
    }
...
cga_putc(char ch) {
    ...
    if (ch == '\b') {
        ...
        // fill with spaces
        pv += pos;
        for (uint32_t i = 0; i < spaces; ++i, ++pv)
            *pv = tch;
 ...
 */
void
cga_putstr(const char *str, uint32_t strlen) {
